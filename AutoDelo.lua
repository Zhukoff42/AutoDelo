...
-- ==========================================
-- УЛУЧШЕННАЯ ЛОГИКА ОБНОВЛЕНИЯ
-- ==========================================
function check_update()
    update_state = "Проверка обновлений..."
    -- Уникальное имя временного файла, чтобы избежать ошибки "device or resource busy"
...
local render_update_window = imgui.OnFrame(function() return update_available end, function(player)
    ...
    -- Кастомные цвета только для этого окна
    ...
    if imgui.Begin("UpdateWindow", nil, flags) then
        ...
        -- Заголовок
        imgui.TextColored(imgui.ImVec4(0.3, 0.6, 1.0, 1.0), u8"🌟 ДОСТУПНО НОВОЕ ОБНОВЛЕНИЕ AUTO-DELO")
        imgui.Separator()
        ...
        -- Статус-бар скачивания
        if update_state:find("Скачивание") or update_state:find("Установка") then
            imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1.0), u8"⏳ " .. u8(update_state))
        else
...
function main()
    ...
    -- Проверка обновлений при заходе
    check_update()
    ...
    while true do
        ...
        -- Автоматический релоад после установки обновы
        if need_reload then
            wait(500)
